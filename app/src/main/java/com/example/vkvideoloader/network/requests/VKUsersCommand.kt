...
class VKUsersCommand(private val uids: IntArray = intArrayOf()) : ApiCommand<List<VKUser>>() {
    override fun onExecute(manager: VKApiManager): List<VKUser> {
        if (uids.isEmpty()) {
            ...
            val call = VKMethodCall.Builder()
                ...
                .build()
            return manager.execute(call, VkUserResponseApiParser())
        } else {
            ...
            for (chunk in chunks) {
                val call = VKMethodCall.Builder()
                    ...
                    .version(manager.config.version)
                    .build()
                result.addAll(manager.execute(call, VkUserResponseApiParser()))
            }
            return result
        }
    }
    ...
    private class VkUserResponseApiParser : VKApiResponseParser<List<VKUser>> {
